---
permalink: /publication_new/
title: "Publications"
author_profile: true
---

### Conference
* PICELF: An Automatic Electronic Layer Layout Generation Framework for Photonic Integrated Circuits (*First PIC electrical routing tool*)  
  **Xiaohan Jiang**, Yinyi Liu, Peiyu Chen, Wei Zhang, Jiang Xu*  
  2025 Design, Automation and Test in Europe Conference (DATE)    [paper](https://ieeexplore.ieee.org/document/10993137)
* PICopilot: An LLM-based Agentic Framework for Assisting Photonic Integrated Circuit Design via Script Generation (*First PIC design script generation tool*)  
  **Xiaohan Jiang**, Zeyu Li, Wei Zhang, Jiang Xu*  
  2026 International Conference on Computer-Aided Design (ICCAD) [paper](https://arxiv.org/abs/2608.01791)
* PICPlace: An Automatic Photonic Integrated Circuit Placement Framework via Reinforcement Learning (*First general PIC placement tool*)  
  **Xiaohan Jiang**, Peiyu Chen, Yinyi Liu, Wei Zhang, Jiang Xu*  
  2026 International Conference on Computer Design (ICCD)
